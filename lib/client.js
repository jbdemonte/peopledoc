...
function Client(peopleDoc) {
  ...
  function _send(query, callback, accepted, retry) {
    request(query, function (err, res) {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300 || accepted[res.statusCode]) {
        ...
        if (res.errors && res.errors.length && res.errors[0].msg) {
          message = res.errors[0].msg + (res.errors[0].code ? ' (code ' + res.errors[0].code + ')' : '');
        } else {
   ...
   * @param {string} path
   * @param {Object} [data]
   * @param {String} [file] - path to file to send
   * @param {Function} [callback]
   */
