module.exports = function (peopleDoc) {
  ...
  // statics
  Employee.RegistrationReference = require('../utils/classBuilder')('registrationReference', peopleDoc);
...
function register(technical_id, registration, callback) {
  var self = this;
  return this.peopleDoc.run(callback, function (resolve, reject) {
    self.peopleDoc.client.post('employees/' + technical_id + '/registrations/', new self.RegistrationReference(registration), function (err) {
      if (err) {
        return reject(err);
      }
      resolve();
