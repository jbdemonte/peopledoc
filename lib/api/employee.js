...
function findById(technical_id, callback) {
  ...
  self.peopleDoc.client.get('employees/' + technical_id + '/', function (err, response) {
    if (utils.evaluate(defer, err, response, {404: true})) {
      defer.resolve(response.json ? new self.peopleDoc.Employee(response.json) : undefined);
    }
  });
...
function register(technical_id, registration, callback) {
  var self = this;
  var defer = self.peopleDoc.defer(callback);
  this.peopleDoc.client.post('employees/' + technical_id + '/registrations/', new self.peopleDoc.RegistrationReference(registration), function (err, response) {
...
function unregister(technical_id, organization_code, registration_number, callback) {
  var self = this;
  var defer = self.peopleDoc.defer(callback);
  this.peopleDoc.client.delete('employees/' + technical_id + '/registrations/' + organization_code + '/' + registration_number, function (err, response) {
    if (utils.evaluate(defer, err, response)) {
      defer.resolve();
    }
  });
...
function leave(technical_id, callback) {
  var self = this;
  var defer = self.peopleDoc.defer(callback);
  this.peopleDoc.client.put('employees/' + technical_id + '/gone/', function (err, response) {
    if (utils.evaluate(defer, err, response)) {
      defer.resolve();
    }
  });
...
function returns(technical_id, callback) {
  var self = this;
  var defer = self.peopleDoc.defer(callback);
  this.peopleDoc.client.put('employees/' + technical_id + '/back/', function (err, response) {
    if (utils.evaluate(defer, err, response)) {
      defer.resolve();
    }
  });
