 ...
 * @param {Function} accessor - Function or Class to cast to
 * @param {Boolean} instantiate - Either or not the accessor is a class to instantiate
 */
function decoreToCast(instance, names, accessor, instantiate) {
  names.trim().split(/\s+/).forEach(function (name) {
    ...
    instance[name] = function () {
      var args = Array.prototype.map.call(arguments, function (value) {
        return instantiate ? new accessor(value) : accessor(value);
      });
 ...
 * @param {Function} accessor - Function or Class to instantiate
 * @param {Boolean} instantiate - Either or not the accessor is a class to instantiate
 * @returns {SetAccessor}
 */
function buildArrayAccessor(accessor, instantiate) {
  return function (inArray) {
    var result = [];
    decoreToCast(result, 'push unshift', accessor, instantiate);
  ...
  Object.keys(description).forEach(function (name) {
    ...
    if (Array.isArray(rule)) {
      if (typeof rule[0] === 'string') {
        setters[name] = buildArrayAccessor(buildSetAccessor(rule[0].trim(), name))
      } else {
        setters[name] = buildArrayAccessor(build(rule[0], peopleDoc, path + '.' + name), true)
      }
